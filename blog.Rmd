...
```{r alltestrating}
    ...
    # Do a t.test
    t.bankrupt <- t.test(allDT[[1]][Bankrupt==T]$Rating,allDT[[1]][Bankrupt==F]$Rating,var.eq=T)
    t.bankrupt
    
    # Find mean difference in two groups
    real <- mean(allDT[[1]][Bankrupt==T]$Rating)-mean(allDT[[1]][Bankrupt==F]$Rating)
    ...
    # Bootstrapping function that calculates the mean difference for each bootstrapped sample
    mean.diff <- function(data,indices) {
        data <- data[indices,]
        mean(data[1:nT]$Rating) - mean(data[(nT+1):nF]$Rating)
    }
    
    # Perform bootstrap using the boot library
    myBoot <- boot(data=allDT[[1]], statistic=mean.diff ,R=n)
    str(myBoot)
    
    # In which quartile is the mean difference? (More or less the same as non-bootstrapped t-test)
    (length(myBoot$t[abs(myBoot$t)>=real])+1)/(n+1)
